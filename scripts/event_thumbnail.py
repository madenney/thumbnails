...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate a full event VOD thumbnail using video_tools.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--event", required=True, help="Event name")
    parser.add_argument("--tagline", help="Optional tagline")
    parser.add_argument("--date", help="Optional event date text")
    parser.add_argument(
        "--output-dir",
        default="output/event_thumbnail",
...
def main(argv: list[str] | None = None) -> int:
    ...
    slug_source = args.slug or args.event
